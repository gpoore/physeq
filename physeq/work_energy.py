...
from astropy.units.si import joule  # type: ignore
...
W = Symbol(r'W', 'work', joule, r'W_{<n>}')
ΣW = sum_W = Symbol(r'\Sigma W', 'sum of work', joule, r'\Sigma W_{<n>}')
...
KE = Symbol(r'\text{KE}', 'kinetic energy', joule, r'\text{KE}_{<n>}')
...
ΔKE = delta_KE = Symbol(r'\Delta\text{KE}', 'change in kinetic energy', joule, r'\Delta\text{KE}_{<n>}')
eq_ΔKE = eq_delta_KE = Eq(ΔKE, KE - KE_0)

PE = Symbol(r'\text{PE}', 'potential energy', joule, r'\text{PE}_{<n>}')
PE_0 = PE.subscript(r'0', description_prefix='initial', subscriptable=True)
ΔPE = delta_PE = Symbol(r'\Delta\text{PE}', 'change in potential energy', joule, r'\Delta\text{PE}_{<n>}')
